...
**Coverage boundary.** 50 of the 115 medications carry `CL`, `Vd` and `ka`, which is what `/api/medications/pk-complete` reports. Of those, 21 have an active metabolite whose volume of distribution is assumed equal to the parent, leaving **29 medications with no substituted value anywhere**.

Molecular weights, needed for every conversion of `Ki` and `Km` between µM and mg/L, are now sourced from PubChem for all 114 single-entity medications and recorded with their CID in `backend/services/molecular_weights.py`. Only one entry cannot resolve: a combination product with no single molecular weight, which returns nothing rather than a meaningless number.

The remaining 65 medications have interaction, CYP450 and half-life data but no volume of distribution, absorption rate constant or bioavailability. Those are genuine pharmacokinetic quantities that need literature work rather than a lookup.

Every substitution the engine makes is recorded and returned with the simulation as `parameter_substitutions`, alongside a `has_substituted_parameters` flag, so a curve built on defaults is distinguishable from one built on measured data. Values legitimately derived from other measurements, such as clearance computed as `ln 2 · Vd / t½`, are marked as derived and do not count as substitutions. Passing `strict=True` refuses to substitute at all. Those entries are complete for the interaction and risk analyses, which is what they are used for, but cannot drive the compartmental model. Half-life = ln 2 · `Vd`/`CL` is one equation in two unknowns, and `ka` additionally needs `tmax`, so the missing parameters are not recoverable from what is present. Rather than invent values, `/api/medications/pk-complete` reports which medications support the PK-model analyses, the search response carries a `has_pk_parameters` flag, and the Design and Diagnostics panels select a usable medication themselves.
