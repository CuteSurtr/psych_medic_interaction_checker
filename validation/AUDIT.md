...
#### F-25. PK coverage is roughly a third of what the README claims  `MOLECULAR WEIGHTS RESOLVED 2026-08-15`

> **Resolution of the molecular weight half.** All 79 missing weights were
> retrieved from PubChem by name lookup and are recorded in
> `services/molecular_weights.py` with their CID, so any value can be
> re-checked. The 36 previously hard-coded values were re-verified at the same
> time and all agreed to within 1%, so none was wrong, only unsourced.
>
> The module is generated by script rather than transcribed. Hand-copying 114
> numbers is exactly the step that produces the errors this audit keeps finding.
>
> Free-base weights are used throughout, which is correct because plasma
> concentrations are reported as the active moiety. Three entries needed
> judgement rather than a lookup: `lithium` uses the ion (6.94) not the
> carbonate (73.89); `amphetamine salts` uses the amphetamine free base;
> and `ethinyl estradiol with norethindrone` is a combination product that
> returns `None` rather than a meaningless single value, so it stays flagged.
>
> | | before | after |
> |---|---:|---:|
> | Molecular weight fabricated | 79 | **1** |
> | Medications needing no fabricated value | 17 | **29** |
>
> **The remaining gap is real and unchanged.** 65 medications still lack volume
> of distribution, absorption rate constant and bioavailability. Those are
> genuine pharmacokinetic quantities requiring literature work, not a chemistry
> lookup, and no equivalent shortcut exists.
>
> The counts now reconcile: 50 medications have complete parent PK, matching the
> router's figure. Of those, 21 have an active metabolite whose volume of
> distribution is still assumed equal to the parent (F-7, open), leaving 29 with
> no fabricated value anywhere.

#### F-25 (original finding)
